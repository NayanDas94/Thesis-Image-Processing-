...
for p = 1:length(myFile6)
    ...
    for q = 1:256
        ...
        for i = ceil((r-1)/3):floor((2*(r-1))/3)
            for j = 2:floor((c-1)/2)
                if img_lbp(i,j) == q-1
                    count = count+1;
                end
...
for p = 1:length(myFile7)
    ...
    for q = 1:256
        ...
        exp_7(p,q+(256*4)) = count;
        
    end
    exp_7(p,(256*5)+1) = f7(p,2)+1;
end
...
for i = 1:A
    fprintf(Eid, '%d ', [exp_7(i,(256*5)+1)]);
    for j=1:B-1
        fprintf(Eid, '%d:%d ', [ j exp_7(i,j)]);
    end
    fprintf(Eid, '\n');
end
